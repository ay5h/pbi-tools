...
class Portal:
    def __init__(self, env, token):
        ...
        self.env = env
        self.api_url = (
            f"https://diageo-eun-orion-datagovernanceapi-{env}.azurewebsites.net"
        )
        self.get_reports()
        self.get_envs()
    ...
    def get_reports(self):
        r = requests.get(
            f"{self.api_url}/admin/report-detail", headers=self.get_headers()
        )
    ...
    def get_report_configs(self):
        r = requests.get(
            f"{self.api_url}/admin/report-configuration", headers=self.get_headers()
        )
        json = handle_request(r)
    ...
    def get_envs(self):
        r = requests.get(
            f"{self.api_url}/admin/report-environment", headers=self.get_headers()
        )
        handle_request(r)
    ...
    def sync_report(self, report, stage, tag=None):
        ...
        if (
            len(matching_reports) > 0
        ):  # If report exists on Portal, add it to API call (to trigger update rather than insert)
            payload["PowerBIReportId"] = matching_reports[0]["Id"]
        else:
            print(f"! Warning: Adding {report_name} to {self.env} Portal")

        r = requests.put(
            f"{self.api_url}/admin/report-configuration",
    ...
    def get_restriction_types(self):
        r = requests.get(
            f"{self.api_url}/admin/user-restriction-type", headers=self.get_headers()
        )
    ...
    def create_user(self, email, first, last, type_key, restrictions):
        ...
        r = requests.post(
            f"{self.api_url}/admin/user", headers=self.get_headers(), json=payload
        )
    ...
    def update_user(self, id, email, first, last, type_key, restrictions):
        ...
        r = requests.put(
            f"{self.api_url}/admin/user/{id}", headers=self.get_headers(), json=payload
        )
